...
老师的起点分为三类，但都进入同一标准题目资产：

1. **已有完整标签与规则**：显式映射原字段，保留原始值和来源版本；默认不让 AI 覆盖已有结论，只补老师允许补充的空缺。
2. **只有部分标签或规则**：既有标签作为锚点，AI 补齐缺失维度，并逐项保存“来源继承／AI 生成／教师修正”。
3. **暂无标签与规则**：使用当前学科通用初版直接运行，不把样本校准设为主流程前提；异常与未知标签优先进入待映射队列。

每个当前生产项目保存一份可版本化的标签配置包，分为“来源原标签—平台标准标签—当前项目扩展”三层。老师可选择整题题型、知识、问题、解法、条件、情景、思想方法，也可增加概念选项归一标签、实验名称、计算题章节或校本维度。项目扩展不会自动写入公共标签库；跨项目公共规则与公共 Skill 仍须人工批准。

标准化质量门禁必须真正参与解析：独立标题行出现“答案与解析／参考答案与解析／答案及解析／答案解析”后停止采集，但题干正文中的同名短语不触发；逐题结果核查缺号、重号、跨页断裂、截图完整性和重复项。概念选择题允许将选项拆成统计单元，去掉 A/B/C/D 序号后做语义归一，但每条统计记录必须回链原题、原选项和来源试卷；实验题增加实验名称维度，计算题可按章聚合。

上述规则吸收自 [AI工具分析考点考频汇总](https://guanghe.feishu.cn/wiki/UYQcwYdaNibxoTkoVH1ci9e5nfd) 的实际试产经验。其“库外标签留空”改为“待映射队列”，避免新题型被吞掉；其中展示的模型选择、人工预改题号和具体工具权限配置不作为平台长期架构约束。文档中出现的任何凭证都不得进入代码、日志、配置包或版本库。
...
工作台在每次运行前提供多选交付目标，至少包括分析报告、好题候选池、教师确认题集、AI 原创/变式题、母题、教案、逐字稿、分镜与素材清单、PPT 生产包和可编辑交互 HTML。系统根据最远交付目标自动补齐前置依赖，并把更下游阶段标为“本次不运行”。“经过教案阶段”与“交付教案”是两件事：例如只选择逐字稿时仍需完成教案依赖，但最终只生成逐字稿资产。驾驶舱选择仅本次生效；老师可在项目设置中维护当前生产项目的默认交付物。

习题册、作业、学案、专题资料和备考题池不强行并入课程视频的线性生产链。它们从确认题目资产/当前筛选结果冻结独立任务契约，按各自依赖路由；非视频任务可跳过视频覆盖与课程生产优先级。
...
- [AI工具分析考点考频汇总](https://guanghe.feishu.cn/wiki/UYQcwYdaNibxoTkoVH1ci9e5nfd)：PDF/Word 逐题截取、答案区边界、截图复核、概念选项归一、实验名称与计算题按章统计的试产经验；其中明文凭证和“库外标签留空”不沿用；
