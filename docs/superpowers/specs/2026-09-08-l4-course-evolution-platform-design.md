...
以下情况不受自动化偏好影响，必须进入异常队列或人工批准：原题图表/关键证据缺失，答案、解析或科学结论冲突，授权或许可不清，对外发布与人员触达条件不完整，以及把项目规则升级为跨项目公共规则或公共 Skill。AI 可自动生成并使用项目内的隔离实验规则继续走跨年度回测；实验版本只有达到门槛后，才能按项目发布策略升级为正式生产版本，跨项目规则与公共 Skill 始终需要人工批准。
...
16. **项目设置**：Base/PMO/素材库/CB 连接、字段映射、成熟度、各节点“自动推进/只看异常/每次确认”、作用域、项目规则自动发布门槛、默认任务和默认生产偏好。
...
29. 教师可逐节点选择“自动推进、只看异常、每次确认”，并设置仅本次、当前生产项目或个人默认；未选为人工确认的正常节点不会等待教师。
...
32. 项目内实验规则与正式生产规则、跨项目公共规则分层治理；公共规则和公共 Skill 不会被自动发布，所有正式版本可追溯和回退。
