...
教师确认契约后再执行。自定义任务默认只读取标准题目资产并生成新产物，不改动权威标签；事实性纠错可进入标准化反馈，个人编排偏好只沉淀到该任务模板或当前生产项目。
...
| 表 | 核心字段 |
|---|---|
| 生产项目 | 项目 ID、学科、范围、目标学生层、负责人、生产规则版本、宣传规则版本、PMO/Base/素材库配置、成熟度、默认首个任务和默认生产偏好 |
...
| 标签字典与映射 | 标签 ID、维度、名称、父标签、来源 Base/字段/选项、规范标签、版本、启停状态和映射置信度 |
| 频次与趋势 | 题目单元/底层结构、统计范围、试卷分子/分母、频次率、题目数、地区数、连续年份、分值、趋势、去重口径、理由、置信度和规则版本 |
| 题目质量 | 题目单元、AI/教研好题结论、质量维度、理由、风险、证据、置信度、审核人和规则版本 |
| 题目集合 | 集合 ID、名称、筛选条件、冻结题目单元、来源批次、创建人、版本和用途 |
| 下游任务 | 任务 ID、模板/自定义类型、题目集合版本、任务契约、依赖模块、规则/模板版本、审核方式、状态和输出资产 |
...
5. **趋势与课程缺口**：并排显示宣传相似与生产覆盖、逐小问/得分点证据、视频证据级别、未覆盖台阶、趋势和置信度。
...
13. **备考候选池**：按考试类型、年级、学科等聚类，核验 CB ID 并下载分层交接包。
14. **分镜与生产路由**：PPT/HTML、生产方式、审核方式和偏好范围。
15. **规则实验室**：反馈聚类、规则候选、回测、批准和回退；生产与宣传使用独立规则域。
16. **项目设置**：Base/PMO/素材库/CB 连接、字段映射、成熟度、默认任务和默认生产偏好。
