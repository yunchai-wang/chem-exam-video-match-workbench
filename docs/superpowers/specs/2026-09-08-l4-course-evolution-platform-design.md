...
选择保存在资料批次上，可继承生产项目默认值。老师可在分析开始前修改；已经生成结果后，不能把原结果原地改成另一口径，但可点击“追加另一分析用途”，从同一冻结题目资产创建新的分析运行，无需重新上传。每次运行保存独立的用途、规则版本、输入快照、教师结论和输出链接。
...
| 表 | 核心字段 |
|---|---|
| 生产项目 | 项目 ID、学科、范围、目标学生层、负责人、生产规则版本、宣传规则版本、PMO/Base/素材库配置、成熟度、默认分析用途 |
| 资料批次 | 批次 ID、分析用途、来源类型、位置、快照、导入状态、异常数、授权会话和检索条件 |
| 分析运行 | 运行 ID、批次 ID、用途、输入快照、生产/宣传规则版本、模板版本、状态、创建人和时间 |
...
2. **新建任务与资料批次**：先选课程生产诊断/押题宣传/两套并行，再进行多源选择、字段映射、菁优网/CB 连接、授权会话和导入预览。
...
6. **宣传匹配与终审**：独立展示题图—视频截图、白名单、相似点、命中题数/分值和宣传文档生成；顶部持续显示宣传口径。
7. **好题与优先级审核**：按题目单元和目标学生层查看难度、卡点、学习收益、缺口、最小干预方式，执行选择、拆分、排除、排序和批量通过。
8. **母题工作台**：原题并排、底层结构、组合建议、来源映射和教师确认。
9. **内容生产**：教案/逐字稿多轮反馈、差异和确认版。
10. **资产中心**：阶段产物、Word/飞书、回传、版本和继续生产。
11. **考情与规则仪表盘**：跨年度预测回测、地区/年份/题型对比、规则版本增益和证据题墙。
12. **备考候选池**：按考试类型、年级、学科等聚类，核验 CB ID 并下载分层交接包。
13. **分镜与生产路由**：PPT/HTML、生产方式、审核方式和偏好范围。
14. **规则实验室**：反馈聚类、规则候选、回测、批准和回退；生产与宣传使用独立规则域。
15. **项目设置**：Base/PMO/素材库/CB 连接、字段映射、成熟度、默认分析用途和默认生产偏好。
...
- 新建任务默认“课程生产诊断”，可选“押题宣传物料”或“两套口径并行”；追加另一用途时复用冻结题目资产，不要求重新上传；
...
- 宣传分支复用现有 v5 提示词/规则与左右双栏文档格式，完成教师白名单终审、逐卷文档和宣传汇总文档；
...
2. 新建任务默认生产诊断，可选宣传或两套并行；同一标准题目资产能追加另一用途，两个运行拥有独立规则版本、状态、反馈和输出。
3. 每道含图题在题目资产、Word/飞书教案和后续分镜中均能回指原图；缺图时系统阻断。
4. 教师可按整题、小问或选项确认候选，排除理由和优先级修改可追溯。
5. 好题和生产优先级分别显示结论、证据和置信度。
6. 宣传相似与生产覆盖使用独立字段和规则版本；课程缺口、覆盖率和生产优先级只读取生产覆盖。
7. 宣传分支能按锁定的 v5 规则/模板生成逐卷双栏对比和汇总，发布前教师可审核白名单、截图、说明和统计分母。
...
13. 每个候选显示“完整新课、旧课改造、短讲/补丁、题解/知识卡、仅题库保留”中的最小充分干预方式。
...
22. CB 题目只有核验后才带 ID；备考候选池可按学科、年级和考试类型筛选并下载直用/待录入/兜底包。
23. 规则候选未经保留集回测和人工批准不能成为正式规则；正式版本可回退。
24. 真实敏感数据不会进入 GitHub，脱敏样例可独立启动演示。

## 22. 实施顺序建议

1. 拆分当前单文件工作台，先建立项目、批次、标准题目、题目单元和反馈事件边界；
2. 实现分析用途选择、独立分析运行，以及文件/文件夹/Base/CB 导出接入、来源 ID 和统一标准化；
3. 实现生产诊断、宣传审计两套隔离规则域，以及题目单元、视频证据工作台和批量通过；
...
9. 接入现有 v5 宣传审计资产，生成逐卷双栏文档、宣传汇总和独立终审记录；
10. 最后接入菁优网与 CB 授权浏览器演示，并把正式 API/导出接口作为独立适配器。
