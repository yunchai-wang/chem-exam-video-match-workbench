...
- 状态：需求边界已确认，补充视频证据、趋势回测与 CB 交接设计
- 日期：2026-09-09
...
| 备考候选池 | 题池 ID、题目单元、学科、年级、考试类型、地区、年份、趋势/好题/频次、CB 身份状态、导出状态、确认人和版本 |
...
2. **新建资料批次**：多源选择、字段映射、菁优网/CB 连接、授权会话和导入预览。
3. **标准化复核**：原文/原图与结构化结果并排，集中处理异常。
4. **趋势与课程缺口**：并排显示宣传相似与生产覆盖、逐小问/得分点证据、视频证据级别、未覆盖台阶、趋势和置信度。
5. **题目—视频证据工作台**：原题、AI 六项门禁和视频截图/逐字稿三栏联动；缺资料时切换人工打标与待补资产。
6. **好题与优先级审核**：按题目单元和目标学生层查看难度、卡点、学习收益、缺口、最小干预方式，执行选择、拆分、排除、排序和批量通过。
7. **母题工作台**：原题并排、底层结构、组合建议、来源映射和教师确认。
8. **内容生产**：教案/逐字稿多轮反馈、差异和确认版。
9. **资产中心**：阶段产物、Word/飞书、回传、版本和继续生产。
10. **考情与规则仪表盘**：跨年度预测回测、地区/年份/题型对比、规则版本增益和证据题墙。
11. **备考候选池**：按考试类型、年级、学科等聚类，核验 CB ID 并下载分层交接包。
12. **分镜与生产路由**：PPT/HTML、生产方式、审核方式和偏好范围。
13. **规则实验室**：反馈聚类、规则候选、回测、批准和回退。
14. **项目设置**：Base/PMO/素材库/CB 连接、字段映射、成熟度和默认偏好。
...
R24：只使用截至 2024 年的数据和已批准规则 → 冻结 2025 候选集合
...
未来年份验证产生的是“验证事件”，先进入误判/漏判案例池，再由 AI 归纳规则候选。候选需说明旧规则为何错、建议修改、适用范围、支持样本、反例和对历史/保留集的影响。只有人工批准后才发布新版本；旧版本、当时预测和原始验证结果永久保留，从而能在仪表盘中真实展示规则进步，而不是展示被改写后的历史。
...
- 提供 Word、飞书协作和资产中心；
- 提供按学科、年级、考试类型聚类的备考候选池，以及 CB 直用清单、外部题 Word 包和图片/PDF 兜底包；
...
- 与菁优网开放平台建立正式数据合作；
- 接入真实学生观看与练习数据；
- 在选题规则、内容/CB ID、上线字段、审核责任、露露团队实际流程和回退机制全部明确后，再申请扩展“备考工具自进化”分支：教师确认题池 → CB 同题核验 → 缺题录入队列 → 备考工具候选配置 → 专职审核 → 上线 → 学生/教师反馈；
...
7. “库中是否有视频”与“生产覆盖”字段分离；仅人工确认视频存在不会被统计为充分覆盖。
8. AI 的覆盖结论可下钻到原题内容块、视频截图/时间码、逐字稿片段和六项门禁；证据不足时显示 E0～E2 并禁止自动充分覆盖。
9. 生产项目明确目标学生层；难度不直接决定优先级，未覆盖但对目标学生增益低的基础题可进入 P3/不生产。
10. 每个候选显示“完整新课、旧课改造、短讲/补丁、题解/知识卡、仅题库保留”中的最小充分干预方式。
11. 母题整合保留原题来源映射，并经过教师确认。
12. 工作台反馈和 Word/飞书回传均能生成版本差异并形成当前确认版。
13. 每个审核队列支持安全的批量通过，硬阻断项不会被放行，批次操作可撤销。
14. 新项目默认逐项校准；满足配置门槛后才可升级，严重错误会自动降级。
15. 教案和逐字稿的规则候选分别进入各自 Skill，不跨域直接改写。
16. PPT 首次默认先确认；确认后可保存为项目自动模式。负责人优先取 PMO，缺失时暂停选择。
17. HTML 不触达兼职，产出源码、运行包和验收结果并回填飞书。
18. 跨年度回测保存冻结预测和未来验证快照；仪表盘显示分子/分母、样本量、误判、漏判和证据题墙，不能用改写后的历史冒充进步。
19. CB 题目只有核验后才带 ID；备考候选池可按学科、年级和考试类型筛选并下载直用/待录入/兜底包。
20. 规则候选未经保留集回测和人工批准不能成为正式规则；正式版本可回退。
21. 真实敏感数据不会进入 GitHub，脱敏样例可独立启动演示。

## 22. 实施顺序建议

1. 拆分当前单文件工作台，先建立项目、批次、标准题目、题目单元和反馈事件边界；
2. 实现文件/文件夹/Base/CB 导出接入、来源 ID 和统一标准化；
3. 实现诊断、候选审核、题目单元、视频证据工作台和批量通过；
4. 接入母题、教案、逐字稿及现有 Skills；
5. 实现资产中心、Word/飞书往返和版本差异；
6. 实现分镜、PPT/HTML 路由、PMO 状态和通知；
7. 实现冻结预测、跨年度滚动回测、可交互仪表盘、证据题墙和规则实验室；
8. 实现备考候选池、CB ID 核验状态和 Word/图片/清单导出；
9. 最后接入菁优网与 CB 授权浏览器演示，并把正式 API/导出接口作为独立适配器。
