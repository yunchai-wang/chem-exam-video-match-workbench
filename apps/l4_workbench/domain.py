...
def migrate_state(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if version == 12:
        ...
        version = 13
    if version == 13:
        state.setdefault("project", {}).setdefault("lesson_type", "problem")
        state["schema_version"] = 14
        version = 14
    if version == 14:
        state.setdefault("ai_tag_fill_runs", [])
        state["schema_version"] = 15
        version = 15
    if version == 15:
        state.setdefault("video_evidence_index_snapshots", [])
        state["schema_version"] = 16
        version = 16
    if version == 16:
        state.setdefault("coverage_candidate_exclusions", [])
...
def validate_state(state: dict[str, Any]) -> None:
    ...
    from .skill_routing import normalize_lesson_type
    normalize_deliverables(project.get("default_deliverables"))
    normalize_lesson_type(project.get("lesson_type"))
    ...
    for key in (
        ...
        "ai_tag_fill_runs",
        "video_evidence_index_snapshots",
        "coverage_candidate_exclusions",
