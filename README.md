# Exam Video Match Workbench / L4 真题驱动的课程自进化平台

中考化学试卷与洋葱视频课的相似题审计工作台。

2026-09-17 质检修复：候选题与母题分组分为独立页签；自动标签映射禁止字符串包含与一对多随意取值，规则辅助补标保持待校准；母题刷新和下游教案按输入版本关联，教师介入遵循项目设置。逐字稿索引补读新教材 PMO 字段与合集附件，定稿/录音稿按明确日期排序，未知先后显式标注。详见 `docs/handoff/CURRENT.md` 顶部；执行包仍需真实生成执行器，不代表已经完成正式课程生产。
...
- 当前项目标签接入向导：支持“已有完整标签、只有部分标签、暂无标签”三种起点，保存来源原标签／平台标准标签／项目扩展三层配置；无标签也能运行，未知标签进入待映射队列而不是被静默丢弃；
- 标准题目资产的内容块、来源定位、指纹去重、图片完整性与异常清单；
- 解析质量门禁会在独立标题行识别到“答案与解析”等答案区标题后停止采集，题干中偶然出现同名短语不会误触发；配置同时外显题号、题图绑定和重复项核查规则；
- 初中化学六维标签契约与版本快照：整题只保留一个宏观题型，小问使用问题任务多标签；全部/核心/前置工具/干扰项/仅提及知识、问题、解法、条件、情景和思想方法分别保存；逐小问标签只读取显式映射，不把整题标签盲目下沉；
- 习题、解题课视频、概念课视频使用不同的知识点作用域契约；相似题实验中的“整题/小问偏好”只作为推荐粒度参考，不作为题型分类或视频覆盖的硬门禁；
- 首轮真实教研诊断：只按显式底层结构计算同年跨地区复现度，高频与好题分别给出；单年数据不冒充多年趋势，缺视频或学生证据时不强行生成生产优先级；
- 吸收 AI 试卷分析平台的打标口径，但保持 L4 的“维度不进黑箱、可回测、可筛选”原则：难度量化到 55/65/75/85/95（显式值 > 星级 > 难度词 > 1-5 折算 > 步骤/知识点估算，估算标低置信），认知价值维度以量化难度为准并保留旧阈值兜底；新素材/新形式/新设问/新题型作为创新特征标签，仅供筛选与展示，不计入好题综合分（新奇≠好题）；记忆依赖度（记忆型/记忆+推理/推理·信息提取型）是带证据与置信度的可筛选自动标签，不作否决门、不改好题判级，无答案解析时置信度下调，教师可按项目需求（如 `apply_project_question_filters`）保留或排除记忆型题；
...
- 驾驶舱可多选本次最终交付物：分析报告、候选池、确认题集、原创/变式题、母题、教案、逐字稿、分镜、PPT 或 HTML；系统自动补齐依赖并裁剪后续阶段，依赖阶段不会被误生成成额外交付物；单次选择不改项目默认；
- 分阶段幂等任务记录、失败状态与重试基础；
...
资料解析可选依赖见 `requirements-l4.txt`。没有安装 `pypdf` 或 `openpyxl` 时，系统会显式报告相应解析能力缺失；PDF 页面原图仍优先通过本机 `pdftoppm` 保留，不会静默丢图。当前答案区边界已可执行，但 PDF 跨页逐题裁切、缺号/重号自动定位和中文 OCR 仍属于后续增强，页面会保留质量门禁而不会把整页解析冒充成可靠逐题截图。
