#!/usr/bin/env python3
"""v4.1 final-review visual-promotion matching pipeline for 2026 chemistry papers."""
...
WORK = ROOT / "outputs" / "2026_yt_visual_v41"
DATA = WORK / "data"
DOCS = WORK / "docs"
FEISHU = WORK / "feishu"
...
IMAGE_TOKEN_PATH = FEISHU / "image_tokens_v41.json"
URLS_PATH = FEISHU / "doc_urls_v41.json"
...
def crop_question(pdf_path: Path, paper: str, qnum: int, positions: list[dict]) -> dict:
    ...
    return {"path": str(final), "page": f"{current['page_index']+1}-{end_page+1}" if end_page != current["page_index"] else str(current["page_index"]+1), "status": "PDF原卷裁图v4.1"}
    ...
    for page_index, page in enumerate(doc):
        for block in page.get_text("blocks"):
            ...
            if parts:
                ...
                groups.append({"start": start_q, "end": end_q, "path": str(out), "page": f"{clips[0][0]+1}-{clips[-1][0]+1}", "status": "题组公共材料裁图v4.1"})
    doc.close()
    return groups
...
def build_exam_records() -> list[dict]:
    ...
    for paper, qs in by_paper.items():
        ...
        for q in sorted(qs, key=lambda x: x["qnum"]):
            ...
            out.append(item)
    save_json(DATA / "exam_records_v41.json", out)
    return out
...
def load_transcripts_for_v41() -> list[dict]:
...
def match_transcript_v41(video: dict, transcripts: list[dict]) -> tuple[dict | None, dict]:
    best_name = None
    ranked = []
    ...
    if best_name and best_name[0] >= 0.58:
        ...
        if nscore >= 0.44 and body_score >= 0.24:
            ...
            adopted = None
            status = "弱匹配待人工复核"
            reason = "候选存在一定相似，但按v4.1规则不直接采用为最终标签"
            best_name = (nscore, tr)
        else:
            best_name = (nscore, tr)
    audit = {
...
def build_video_records_and_evidence() -> tuple[list[dict], list[dict]]:
    ...
    transcripts = load_transcripts_for_v41()
    media = old_image.collect_sheet_media()
    videos = []
    evidence = []
    transcript_audits = []
    for v in v3_videos:
        matched_tr, tr_audit = match_transcript_v41(v, transcripts)
        ...
        nv = {
            **v,
            ...
            "project_chain": labels["project_chain"],
            "promotion_visibility": labels["promotion_visibility"],
            "transcript_id_v41": matched_tr.get("transcript_id", "") if matched_tr else "",
            "transcript_file_v41": matched_tr.get("file_name", "") if matched_tr else "",
            "transcript_match_status_v41": tr_audit["adopt_status"],
            "transcript_file_score_v41": tr_audit["file_name_score"],
            "transcript_body_score_v41": tr_audit["body_score"],
    ...
    save_json(DATA / "video_records_v41.json", videos)
    save_json(DATA / "screenshot_evidence_v41.json", evidence)
    save_json(DATA / "transcript_match_audit_v41.json", transcript_audits)
            ...
            matches.append(
                {
                    ...
                    "screenshot_ids": "",
                    "screenshot_notes": "",
                    "reject_reason": "未达到v4.1宣传终审版展示标准",
                    "final_show": False,
                    ...
                    "only_same_chapter": "是",
                    "final_review": "降级过程库",
                    "review_reason": "未达到v4.1宣传终审版展示标准",
                }
            )
    save_json(DATA / "matches_v41.json", matches)
    return matches
...
def upload_images(exams: list[dict], evidence: list[dict], matches: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    save_json(IMAGE_TOKEN_PATH, tokens)
    save_json(DATA / "exam_records_v41.json", exams)
    save_json(DATA / "screenshot_evidence_v41.json", evidence)
...
def build_docs(exams: list[dict], videos: list[dict], evidence: list[dict], matches: list[dict], base_url: str = "") -> list[dict]:
    ...
    for paper, qs in sorted(by_paper.items()):
        ...
        for q in qs:
            ...
            analysis_rows.append(
                [
                    ...
                    escape("；".join(m["video_name"] for m in ms[:3]) if ms else "未达到v4.1展示标准"),
                ]
            )
            if not ms:
                continue
        ...
        base_para = f'<p>过程库：<a href="{escape(base_url)}">2026中考化学押题工作台v4.1（宣传终审版）</a></p>' if base_url else ""
        xml = "\n".join(
            [
                f"<title>洋葱学园 VS {escape(paper)} 押题对比（v4.1宣传终审版）</title>",
                "<h1>基本认识</h1>",
                f"<p>本卷共抽取 {len(qs)} 道题；v4.1 以宣传展示为口径，优先外观相似，外观一般时用设问任务同构或解法模型相似兜底；只同章节不进入最终文档。</p>",
                ...
                "<h1>内容对比表</h1>",
                html_table(compare_rows or [["<p>本卷暂无达到 v4.1 宣传终审标准的题目。</p>", ""]], ["中考题目", "洋葱内容"], [560, 560]),
            ]
        )
        path = DOCS / f"{safe_slug(paper)}_v41宣传终审版.xml"
    ...
    save_json(DATA / "examples_v41.json", examples)
...
BASE_SCHEMA = {
    ...
    "匹配审计库v4.1": [
        ...
        {"name": "多截图分工", "type": "text"}, {"name": "是否仅同章节", "type": "text"}, {"name": "终审结论", "type": "text"}, {"name": "终审理由", "type": "text"}, {"name": "进入最终文档", "type": "checkbox"},
    ],
    "v4.1相似题规则库": [
...
def create_base() -> dict:
    info_path = FEISHU / "base_info_v41.json"
    if info_path.exists():
        return load_json(info_path)
    res = run_json(
        [
            "lark-cli", "base", "+base-create", "--as", "user", "--name", "2026中考化学押题工作台v4.1（宣传终审版）",
            "--time-zone", "Asia/Shanghai", "--table-name", "试卷题库v4.1", "--fields", json.dumps(BASE_SCHEMA["试卷题库v4.1"], ensure_ascii=False), "--format", "json",
        ],
        timeout=240,
    )
...
def rows_for_table(table: str) -> tuple[list[str], list[list[Any]]]:
    if table == "试卷题库v4.1":
        fields = ["题目ID", "试卷", "题号", "一级题型", "视觉形态", "背景素材", "设问任务", "设问任务模型", "解法模型", "核心实验模型", "项目式任务链", "宣传可见性", "PDF裁图", "题干"]
        rows = [[r["question_id"], r["paper"], r["qnum"], r["primary_type"], r["visual_forms"], r["background_tags"], r["task_tags"], r.get("task_models", ""), r["method_models"], r["core_experiment_models"], r["project_chain"], r["promotion_visibility"], r["pdf_crop_path"], r["stem"][:90000]] for r in load_json(DATA/"exam_records_v41.json", [])]
    elif table == "视频截图证据库v4.1":
        fields = ["截图ID", "视频ID", "视频名称", "来源", "来源行号", "截图列", "截图Token", "视觉形态", "背景素材", "设问任务", "设问任务模型", "解法模型", "质检状态"]
        rows = [[r["screenshot_id"], r["video_id"], r["video_name"], r["source"], r.get("source_row") or 0, r.get("screenshot_col", ""), r.get("screenshot_token", ""), r.get("visual_forms", ""), r.get("background_tags", ""), r.get("task_tags", ""), r.get("task_models", ""), r.get("method_models", ""), r.get("quality_status", "")] for r in load_json(DATA/"screenshot_evidence_v41.json", [])]
    elif table == "视频候选库v4.1":
        fields = ["视频ID", "来源", "来源行号", "视频名称", "一级题型", "视觉形态", "背景素材", "设问任务", "设问任务模型", "解法模型", "逐字稿文件", "逐字稿匹配状态", "文件名分", "正文分", "逐字稿证据"]
        rows = [[r["video_id"], r.get("source", ""), r.get("source_row") or 0, r["video_name"], r["primary_type"], r["visual_forms"], r["background_tags"], r["task_tags"], r.get("task_models", ""), r["method_models"], r.get("transcript_file_v41", ""), r.get("transcript_match_status_v41", ""), r.get("transcript_file_score_v41") or 0, r.get("transcript_body_score_v41") or 0, r.get("transcript_evidence", "")[:90000]] for r in load_json(DATA/"video_records_v41.json", [])]
    elif table == "逐字稿匹配审核库v4.1":
        fields = ["视频ID", "来源", "来源行号", "视频名称", "候选逐字稿ID", "候选逐字稿文件", "文件名分", "正文分", "采用状态", "拒绝原因", "匹配说明"]
        rows = [[r["video_id"], r["source"], r["source_row"], r["video_name"], r["candidate_transcript_id"], r["candidate_file"], r["file_name_score"], r["body_score"], r["adopt_status"], r["reject_reason"], r["match_reason"]] for r in load_json(DATA/"transcript_match_audit_v41.json", [])]
    elif table == "匹配审计库v4.1":
        fields = ["匹配ID", "题目ID", "试卷", "题号", "题型", "视频ID", "视频名称", "视频题型", "评分", "命中说明", "截图ID列表", "拒绝原因", "第一眼是否像", "同题型池", "命中说明是否具体", "多截图分工", "是否仅同章节", "终审结论", "终审理由", "进入最终文档"]
        rows = [[r["match_id"], r["question_id"], r["paper"], r["qnum"], r["primary_type"], r["video_id"], r["video_name"], r["video_type"], r["score"], r["hit_reason"], r["screenshot_ids"], r["reject_reason"], r.get("first_eye_similarity", ""), r.get("same_type_pool", ""), r.get("specific_reason", ""), r.get("multi_screenshot_roles", ""), r.get("only_same_chapter", ""), r.get("final_review", ""), r.get("review_reason", ""), r["final_show"]] for r in load_json(DATA/"matches_v41.json", [])]
    else:
        fields = ["规则ID", "规则名称", "规则内容"]
        rows = [
            ["V4.1-01", "宣传版总原则", "优先打眼相似；外观一般时用题型任务/解法模型兜底；只同章节或只同知识点不进最终展示。"],
            ["V4.1-02", "设问任务同构优先", "学生解题动作一致即可判为相似，如共存判断、除杂判断、鉴别判断、图像匹配、方案评价、循环物质判断。"],
            ["V4.1-03", "逐字稿匹配优先级", "先看视频名称与逐字稿文件名相似度；文件名不能确认时再用正文匹配。弱匹配只进审核库，不直接生成最终标签。"],
            ["V4.1-04", "项目式探究题", "先在项目式/跨学科/主题式探究视频池召回；优先结构相似，其次任务链或子任务相似。"],
            ["V4.1-05", "人工终审字段", "最终文档只展示终审结论=保留；只同章节自动降级到过程库。"],
            ["V4.1-06", "题组与跨页", "识别公共材料题组和跨页长题；左列使用完整PDF原卷裁图。"],
        ]
    return fields, rows


def populate_base(info: dict) -> None:
    done = FEISHU / "base_records_v41.json"
    if done.exists():
        return
    status = {}
...
def publish_docs(manifest: list[dict], base_url: str) -> dict:
    ...
    summary_title = "2026 中考化学押题对比汇总（v4.1宣传终审版）"
    if summary_title not in urls:
        ...
        examples = load_json(DATA / "examples_v41.json", [])[:40]
        ex_rows = [[escape(e["paper"]), escape(str(e["qnum"])), escape(e["type"]), escape(e["video"]), escape(e["reason"][:180])] for e in examples]
        xml = "\n".join([
            "<title>2026 中考化学押题对比汇总（v4.1宣传终审版）</title>",
            "<h1>总体说明</h1>",
            "<p>v4.1 宣传终审版优先打眼相似；外观一般时，用设问任务同构或解法模型相似兜底；只同章节或只同知识点不进入最终文档。</p>",
            f'<p>过程库：<a href="{escape(base_url)}">2026中考化学押题工作台v4.1（宣传终审版）</a></p>',
            "<h1>分卷文档链接</h1>",
            html_table(rows, ["试卷", "最终展示命中题数", "按分值估算覆盖"], [420, 140, 140]),
            "<h1>典型案例</h1>",
            html_table(ex_rows or [["暂无", "", "", "", ""]], ["试卷", "题号", "题型", "洋葱证据", "命中说明"], [210, 55, 110, 220, 420]),
        ])
        path = DOCS / "2026中考化学押题对比汇总_v41宣传终审版.xml"
        path.write_text(xml, encoding="utf-8")
...
def verify_docs(urls: dict) -> dict:
    ...
    save_json(FEISHU / "verification_v41.json", checks)
    return checks


def update_source_sheets(base_info: dict, videos: list[dict]) -> None:
    status_path = FEISHU / "source_sheet_updates_v41.json"
    if status_path.exists():
        return
    start_cols = {"新中考培优": "BM", "重难点培优": "BG", "教材同步": "CN"}
    headers = ["AI_v41题型标签", "AI_v41视觉锚点", "AI_v41设问任务模型", "AI_v41逐字稿匹配状态", "AI_v41工作台链接", "AI_v41更新时间"]
    ...
    for source, cfg in prof.SOURCE_WRITE_CONFIG.items():
        ...
        if cfg["header_rows"] == 2:
            rows.append(["AI_v41过程字段"] + [""] * (len(headers) - 1))
            rows.append(headers)
            start_row = 3
        else:
            rows.append(headers)
        ...
        for row_no in range(start_row, cfg["max_row"] + 1):
            ...
            if not v:
                ...
                rows.append([v["primary_type"], v["visual_forms"], v.get("task_models", ""), v.get("transcript_match_status_v41", ""), base_info["base_url"], now])
        buf = io.StringIO()
...
def publish() -> None:
    exams = load_json(DATA / "exam_records_v41.json", None)
    videos = load_json(DATA / "video_records_v41.json", None)
    evidence = load_json(DATA / "screenshot_evidence_v41.json", None)
    matches = load_json(DATA / "matches_v41.json", None)
    ...
    save_json(DATA / "publish_summary_v41.json", {"base": base_info, "urls": urls, "checks": checks})
